...
void borra() {
   ...
   HBRUSH hBrush = CreateSolidBrush(RGB(_color, _color, _color));
...
void linea(float x_ini, float y_ini, float x_fin, float y_fin, float pin) {
   BeginPath(hDCMem);
   MoveToEx(hDCMem, int(x_ini), int(y_ini), NULL);
   LineTo(hDCMem, int(x_fin), int(y_fin));
   EndPath(hDCMem);
   HPEN hPen = CreatePen(PS_SOLID, pin, _color);
   SelectObject(hDCMem, hPen);
   StrokePath(hDCMem);
   DeleteObject(hPen);
}

void linea_p(float x_ini, float y_ini, float x_fin, float y_fin, float pin) {
   BeginPath(hDCMem);
   MoveToEx(hDCMem, int(x_ini), int(y_ini), NULL);
   LineTo(hDCMem, int(x_fin), int(y_fin));
   EndPath(hDCMem);
   HPEN hPen = CreatePen(PS_DASHDOT, pin, _color);
   SelectObject(hDCMem, hPen);
   StrokePath(hDCMem);
   DeleteObject(hPen);
}
...
void rectangulo_r(float izq, float arr, float der, float aba, float jar) {
   HPEN hPen = CreatePen(PS_SOLID, jar, _color);
   HGDIOBJ orig = SelectObject(hDCMem, hPen);
   _rect(izq, arr, der, aba);
   StrokePath(hDCMem);
   SelectObject(hDCMem, orig);
...
void arco(int xi, int yi, int xd, int yd, int xt, int yt, int xc, int yc){
	
	HPEN hPen = CreatePen(PS_SOLID, 1, _color);
   HGDIOBJ orig = SelectObject(hDCMem, hPen);
   
   BeginPath(hDCMem);
	Arc(hDCMem,xi,yi,xd,yd,xt,yt,xc,yc);
	EndPath(hDCMem);
...
void circulo_r(float x_cen, float y_cen, float radio, float ratio) {
   HPEN hPen = CreatePen(PS_SOLID, ratio, _color);
   HGDIOBJ orig = SelectObject(hDCMem, hPen);
   _circ(x_cen, y_cen, radio);
   StrokePath(hDCMem);
   SelectObject(hDCMem, orig);
...
void punto(float x, float y) {
   _lock();
   XDrawPoint(_dsp, _buffer, _bufgc/2, x, y);
   _unlock();
}
